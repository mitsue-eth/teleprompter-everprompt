...
export function EnhancedScriptEditor({
  ...
  const formatHeading1 = () => {
    ...
    const newText =
      localText.substring(0, lineStart) + newLine + localText.substring(lineEndPos)
    handleTextChange(newText)
  }
  ...
  React.useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (modKey && e.key === "b" && !e.shiftKey) {
        ...
        e.preventDefault()
        handleUndo()
      } else if ((modKey && e.key === "y") || (modKey && e.shiftKey && e.key === "z")) {
        e.preventDefault()
        handleRedo()
        ...
        e.preventDefault()
        setMode((prev) => (prev === "distraction-free" ? "normal" : "distraction-free"))
      } else if (e.key === "F11") {
        e.preventDefault()
        setMode((prev) => (prev === "fullscreen" ? "normal" : "fullscreen"))
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      ...
      >
        <DialogHeader className={cn("px-4 py-3 border-b flex-shrink-0", isDistractionFree && "hidden")}>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          {showEditor && (
            <div
              className={cn(
                "flex flex-col",
                viewMode === "split" ? "w-1/2 border-r-2 border-border" : "w-full"
              )}
            >
              {viewMode === "split" && (
          ...
          {showPreview && (
            <div
              className={cn(
                "flex flex-col",
                viewMode === "split" ? "w-1/2 border-l-2 border-border" : "w-full"
              )}
            >
              {viewMode === "split" && (
                <div className="px-4 py-2 border-b border-border bg-muted/30">
              ...
              >
                {localText.trim() ? (
                  ...
                  <div className="text-muted-foreground text-center mt-20">
                    ...
                    <p className="text-sm mt-2">Start typing to see the markdown preview</p>
                  </div>
                )}
              </div>
            </div>
